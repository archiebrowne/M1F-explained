...
lemma part_a (x : ℕ) (hx : x < 24) : ∃ (m : ℕ), x + 7 * m ≡ 9 [MOD 24] := by
  -- Our choice of 'm'
  use 7 * (33 - x)
  -- Lean distinguishes between Integer and Natural modular arithmetic
  rw [← Int.coe_nat_modEq_iff] 
  -- The next three lines simplify the goal, keeping track of the coerscions
  push_cast
  rw [Nat.cast_sub (show x ≤ 33 by linarith)]
  ring_nf 
  -- Apply the modular arithemetic and prove that our choice of 'm' works
